...
def show_warning():
    ...
    tell application "System Events"
        display dialog "NE RÁGDDD!" with title "Figyelmeztetés" buttons {"OK"} default button "OK" giving up after 1
    end tell
    '''
...
class NailBitingDetection():
    ...
    def __init__(self):
        ...
        self.printed = False
        self.music_filename = "resources/error.mp3"
    ...
    def detect(self, email_dict=None):
        while self.cap.isOpened():
            ...
            frame_thread = threading.Thread(target=self.read_frame)
            frame_thread.start()
            frame_thread.join(timeout=1.0)  # Timeout of 1 second

            if frame_thread.is_alive():
                print("Frame read timed out.")
                self.cap.release()
            ...
            if face_results.detections:
                ...
                for detection in face_results.detections:
                    ...
                    cv2.rectangle(self.frame, (mouth_rect_left, mouth_rect_top),
                    ...
                    if hand_results.multi_hand_landmarks:
                        ...
                        for hand_landmarks in hand_results.multi_hand_landmarks:
                            ...
                            palm_center_x = int(sum([lm.x for lm in palm_landmarks]) / len(palm_landmarks) * self.frame.shape[1])
                            palm_center_y = int(sum([lm.y for lm in palm_landmarks]) / len(palm_landmarks) * self.frame.shape[0])
                            ...
                            cv2.rectangle(self.frame, (hand_rect_left, hand_rect_top),
            ...
            if self.countdown_active:
                ...
                if self.time_biting > 0.0:
                    cv2.putText(self.frame, f"Biting for: {int(self.time_biting)}s", (10, 460), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
            else:
                # Displaying no-biting timer
                cv2.putText(self.frame, f"Clean for: {self.time_since_last_bite:.0f}s", (10, 460), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)

            # Displaying nail-biting count
            cv2.putText(self.frame, f"Caught: {self.nail_biting_count} times", (10, 30), cv2.FONT_HERSHEY_DUPLEX, 0.7, (255, 0, 0), 2, cv2.LINE_AA)

            cv2.imshow("Biting Nails Recognition", self.frame)
